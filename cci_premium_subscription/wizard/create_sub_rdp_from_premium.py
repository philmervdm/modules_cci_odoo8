...
class wizard_create_subscription_rdp_from_premium(models.TransientModel):

    _name = 'wizard.create.subscription.rdp.from.premium'
    ...
    def second_form(self):
        ...
        value = {
            ...
            'view_type': 'form',
            'view_mode': 'form',
            'res_model': 'wizard.create.subscription.rdp2.from.premium',
            'context': ctx,
    ...
    def default_get(self,fields):
        rec = super(wizard_create_subscription_rdp_from_premium, self).default_get(fields)
        rec['contact_id'] = self.env.context.get('active_id')
        return rec
        
class wizard_create_subscription_rdp2_from_premium(models.TransientModel):
    _name = 'wizard.create.subscription.rdp2.from.premium'
    ...
    @api.model
    def default_get(self,fields):
        rec = super(wizard_create_subscription_rdp2_from_premium, self).default_get(fields)
    ...
    def create_data(self):
        obj_sub = self.pool.get('premium_subscription')
        (new_id_sub,new_id_invoice,msg_final) = obj_sub._create_new_rdp(self.env.cr,self.env.uid,
                                                                                  self.env.context.get('contact_id'),
...
class wizard_create_subscription_premium_msg(models.TransientModel):
    _name = 'wizard.create.subscription.premium.msg'
    
    msg = fields.Text(string='File imported', readonly=True)
    
    @api.model
    def default_get(self,fields):
        rec = super(wizard_create_subscription_premium_msg, self).default_get(fields)
