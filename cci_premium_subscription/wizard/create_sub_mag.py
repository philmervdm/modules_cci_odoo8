...
class wizard_create_subscription_mag(models.TransientModel):
    
    _name = 'wizard.create.subscription.mag'

    contact_id = fields.Many2one('res.partner',string='Premium Member', required=True)
    product_id = fields.Many2one('product.product',string='Product',required=True)
    duration = fields.Selection([('year','One Year (10 issues)')], string='Duration', default='year', required=True)
    
    @api.model
    def default_get(self,fields):
        res = super(wizard_create_subscription_mag, self).default_get(fields)
    ...
    def second_form(self):
        ...
        value = {
            ...
            'view_type': 'form',
            'view_mode': 'form',
            'res_model': 'wizard.create.subscription.mag2',
            'context': ctx,
...
class wizard_create_subscription_mag2(models.TransientModel):
    _name = 'wizard.create.subscription.mag2'
    
    begin_date =  fields.Date(string='Begin Date', required=True)
    partner_id = fields.Many2one('res.partner',string='Company')
    first_issue_id = fields.Many2one('sale.advertising.issue',string='First Issue Manualy Sended')
    auto_invoice = fields.Boolean(string = 'Make Invoice')
    
    @api.model
    def default_get(self,fields):
        rec = super(wizard_create_subscription_mag2, self).default_get(fields)
        ...
        type_ids = obj_sub_type.search([('code','=','CCIMAG')]).ids
        if type_ids:
            obj_product = self.env['product.product']
            prod_ids = obj_product.search([('premium_subscription_type_id','in',type_ids)])
            if prod_ids:
                obj_sub = self.env['premium_subscription']
                sub_ids = obj_sub.search([('state','=','current'),('product_id','in',prod_ids.ids),('left_usages','>',0)])
                if sub_ids:
    ...
    def create_data(self):
        obj_sub = self.env['premium_subscription']
        (new_id_sub,new_id_invoice,msg_final) = obj_sub._create_new(self.env.context.get('contact_id'),
                                                                  self.env.context.get('product_id'),
                                                                  self.begin_date,
                                                                  self.partner_id.id,
                                                                  self.env.context.get('duration'),
                                                                  self.first_issue_id.id,
                                                                  self.auto_invoice)
        if msg_final:
            msg = msg_final
        else:
        ...
        value = {
            ...
            'view_type': 'form',
            'view_mode': 'form',
            'res_model': 'wizard.create.subscription.mag.msg',
            'context': ctx,
            'type': 'ir.actions.act_window',
            'target': 'new'
        }
...
class wizard_create_subscription_mag_msg(models.TransientModel):
    _name = 'wizard.create.subscription.mag.msg'
        
    msg = fields.Text(string='File imported', readonly=True)
    
    @api.model
    def default_get(self,fields):
        rec = super(wizard_create_subscription_mag_msg, self).default_get(fields)
