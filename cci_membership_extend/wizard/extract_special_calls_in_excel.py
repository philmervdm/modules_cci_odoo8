...
class extract_special_calls_in_excel(models.TransientModel):
    _name = 'extract.special.calls_in_excel'
    ...
    target = fields.Selection([('free', 'Free Members'),
                               ('old', 'Old Members'),
                               ('canceled', 'Canceled Members')],
                              string='Target', default='free', required=True)
    job_categs = fields.Char(string='Categories of searched contacts', size=20, required=True)
    price_site = fields.Char(string='W/o VAT Price per Site', digits=(15, 2), required=True)
    ...
    def open_window_results(self):
        ...
        partners = partner_obj.search([('membership_state', '=', mstate),
                                       ('state_id', '=', 1),
        ...
        ws1.write(0, 32, u'last_year_bill_membership')
        ws1.write(0, 33, u'associated')
        ws1.write(0, 34, u'refuse')
        ws1.write(0, 35, u'desc250')
        line = 1
        for partner in partners:
            if partner.id not in excluded_partner_ids:
                # search the first good address in the order of sequence_address (witch is the order of appearance in partner.child_ids
                ...
                if good_address:
                    ...
                    last_membership_year = ''
                    if partner.member_lines:
                        for mline in partner.member_lines:
                            if mline.account_invoice_line and mline.account_invoice_line.product_id and mline.account_invoice_line.product_id.membership_year:
                                last_membership_year = max(last_membership_year, str(mline.account_invoice_line.product_id.membership_year))
                    # record the data
                    ...
                    ws1.write(line, 32, last_membership_year)
                    ws1.write(line, 33, partner.associate_member and 'Yes' or '')
                    ws1.write(line, 34, partner.refuse_membership and 'Yes' or '')
                    ws1.write(line, 35, partner.membership_explanation and partner.membership_explanation[0:250] or '')
                    line += 1
        wb1.save('calls.xls')
        result_file = open('calls.xls', 'rb').read()

        # give the result tos the user
        view = self.env.ref('cci_membership_extend.view_wizard_extract_special_calls_in_excel_msg_form')
        ...
        return {
            ...
            'view_type': 'form',
            'view_mode': 'form',
            'res_model': 'wizard.extract.special.calls_in_excel.msg',
            'views': [(view.id, 'form')],
            'view_id': view.id,
            'target': 'new',
            'context': ctx
...
class wizard_extract_special_calls_in_excel_msg(models.TransientModel):
    _name = 'wizard.extract.special.calls_in_excel.msg'
    ...
    def default_get(self, fields):
        res = super(wizard_extract_special_calls_in_excel_msg, self).default_get(fields)
        res['name'] = 'calls.xls'
