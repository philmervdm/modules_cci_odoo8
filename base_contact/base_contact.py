...
class ir_actions_window(osv.osv):
    ...
    def read(self, cr, user, ids, fields=None, context=None, load='_classic_read'):
        ...
        if (not fields or (fields and 'context' in fields)):
            model_action = dict((x['id'], x['res_model'])
                                for x in super(ir_actions_window, self).read(cr, user, action_ids, ['res_model'], context=context, load=load))
            for action in actions:
                if model_action[action['id']] == 'res.partner':
